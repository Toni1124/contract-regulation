...
onMounted(async () => {
  ...
  // 如果是从合约详情页跳转来的，自动填充合约地址
  const contractAddress = route.query.contractAddress as string
  if (contractAddress) {
    form.value.contractAddress = contractAddress
    // 获取合约详情
    const contract = contracts.value.find(c => c.address === contractAddress)
    if (contract) {
      selectedContract.value = contract
    }
  }
